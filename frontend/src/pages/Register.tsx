...
export default function Register() {
  ...
  if (success) {
    return (
      <div style={{ maxWidth: 400, margin: '100px auto', padding: 30, background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, textAlign: 'center' }}>
  ...
  return (
    <div style={{ maxWidth: 400, margin: '100px auto', padding: 30, background: 'white', borderRadius: 10, boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
      ...
      <div style={{ background: '#eff6ff', borderLeft: '4px solid #3b82f6', padding: 12, marginBottom: 20, fontSize: '0.9em', color: '#1e40af' }}>
        <strong>Внимание:</strong> Для регистрации необходим уникальный ключ приглашения, который выдает администратор системы.
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input
          name="invitation_key"
          type="text"
          placeholder="Ключ приглашения (напр. NP-xxxxx...)"
          value={formData.invitation_key}
          onChange={handleChange}
          required
          style={{ width: '100%', padding: 10, marginBottom: 10, boxSizing: 'border-box', border: '1px solid #ddd', borderRadius: 5, fontFamily: 'monospace' }}
        />
        ...
        <button 
          ...
          disabled={isLoading}
          style={{ 
            width: '100%', padding: 12, background: isLoading ? '#9ca3af' : '#667eea', 
            color: 'white', border: 'none', borderRadius: 5, cursor: isLoading ? 'not-allowed' : 'pointer', fontWeight: 'bold' 
          }}
        >
