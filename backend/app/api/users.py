...
def list_sessions(
    ...
    sessions = (
        ...
        .filter(SessionModel.user_id == user.id, SessionModel.is_active == True)
        .all()
    )
    return [
        {"id": s.id[:8] + "...", "ip": s.ip_address, "created": s.created_at}
        for s in sessions
    ]
...
def list_tokens(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    tokens = (
        db.query(APIToken)
        .filter(APIToken.user_id == user.id, APIToken.is_active == True)
        .all()
    )
    return [
        {"id": t.id, "name": t.name, "scopes": t.scopes, "created": t.created_at}
        for t in tokens
    ]
